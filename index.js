...
// ── Body parsing helper (replaces express.json()) ──────────────────────────
async function parseBody(c) {
  try { return await c.req.json(); } catch(e) { return {}; }
}

// ── req/res shim so all existing route logic works unchanged ───────────────
function makeReq(c, params, query, body) {
  return {
    params: params || {},
    query: query || {},
    body: body || {},
    ...
    socket: { remoteAddress: c.req.header('cf-connecting-ip') || c.req.header('x-forwarded-for') || 'unknown' }
  };
}
function makeRes() {
  let _status = 200;
...
app.get('/', async (c) => {
  ...
  res.send(buildConfigPage(baseUrl));
...
app.post('/generate', async (c) => {
  ...
  var instanceUrl = (body && body.instanceUrl) ? String(body.instanceUrl).trim().replace(/\/+$/, '') : null;
  if (instanceUrl) {
    if (!/^https?:\/\/.+/.test(instanceUrl))
  ...
  var VALID_QUALITIES = ['LOSSLESS', 'HIGH', 'LOW'];
  var preferredQuality = (body && body.preferredQuality && VALID_QUALITIES.includes(body.preferredQuality)) ? body.preferredQuality : null;

  var token = generateToken();
  var entry = { createdAt: Date.now(), lastUsed: Date.now(), reqCount: 0, rateWin: [], instanceUrl, preferredQuality };
  TOKEN_CACHE.set(token, entry);
  await redisSave(token, entry);
  bucket.count++;
...
app.post('/refresh', async (c) => {
  const body = await parseBody(c);
  var raw = (body && body.existingUrl) ? String(body.existingUrl).trim() : '';
  var token = raw, m = raw.match(/\/u\/([a-f0-9]{28})\//);
  if (m) token = m[1];
...
app.get('/health', (c) => {
  return Response.json({ status: 'ok', version: '2.0.0', activeInstance, instanceHealthy, activeTokens: TOKEN_CACHE.size, redisConnected: false, timestamp: new Date().toISOString() });
});

// token middleware helper for Hono
...
app.get('/u/:token/search', async (c) => {
  return withToken(c, async (entry) => {
    var q = String(c.req.query('q') || c.req.query('query') || c.req.query('s') || '').trim();
    var limit = Math.min(parseInt(c.req.query('limit') || '20', 10) || 20, 50);
    var inst = entry.instanceUrl;
    ...
    try {
      ...
      var albumList = Object.keys(albumMap).map(function(k) { return albumMap[k]; }).slice(0, 8);
      var artistList = Object.keys(artistMap).sort(function(a, b) {
        return (artistRelevance(artistMap[b].name, q) * 100 + (artistHits[b] || 0)) - (artistRelevance(artistMap[a].name, q) * 100 + (artistHits[a] || 0));
      }).slice(0, 5).map(function(k) { return artistMap[k]; });
      ...
      for (var ai = 0; ai < arCandidates.length; ai++) {
        ...
        var realArtists = arItems.filter(function(a) { return a && (a.id || a.artistId) && a.name; });
        if (realArtists.length > 0) {
          artistList = realArtists.sort(function(a, b) { return artistRelevance(b.name, q) - artistRelevance(a.name, q); }).slice(0, 5).map(function(a) {
            return { id: String(a.id || a.artistId), name: a.name, artworkURL: coverUrl(a.picture || a.cover, 320) };
          });
          break;
        }
      ...
      return Response.json({ tracks, albums: albumList, artists: artistList, playlists: plItems });
    } catch (e) {
...
app.get('/u/:token/stream/:id', async (c) => {
  return withToken(c, async (entry) => {
    var tid = c.req.param('id');
    var inst = entry.instanceUrl;
    var ALL_QUALITIES = ['LOSSLESS', 'HIGH', 'LOW'];
    var pref = entry.preferredQuality;
    var qualities = pref ? [pref].concat(ALL_QUALITIES.filter(function(q) { return q !== pref; })) : ALL_QUALITIES;
    for (var qi = 0; qi < qualities.length; qi++) {
      var ql = qualities[qi];
      try {
        var data = await hifiGetForToken(inst, '/track/', { id: tid, quality: ql });
        ...
        if (qi === qualities.length - 1) return Response.json({ error: 'Could not get stream URL for track ' + tid }, { status: 502 });
      }
    }
...
app.get('/u/:token/artist/:id', async (c) => {
  return withToken(c, async (entry) => {
    var aid = parseInt(c.req.param('id'), 10);
    var inst = entry.instanceUrl;
...
app.get('/u/:token/playlist/:id', async (c) => {
  return withToken(c, async (entry) => {
    ...
    try {
      ...
      return Response.json({ id: String((pl && (pl.uuid || pl.id)) || pid), title: (pl && pl.title) || 'Playlist', creator: (pl && pl.creator && pl.creator.name) || undefined, artworkURL: (pl && (pl.squareImage || pl.image)) ? coverUrl(pl.squareImage || pl.image, 480) : undefined, trackCount: (pl && pl.numberOfTracks) || tracks.length, tracks });
